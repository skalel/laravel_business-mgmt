        ...
        <div class="rounded-lg py-8 shadow-md">
          <div class="text-gray-800 dark:text-gray-200">
            <div x-data="{ selectedOption: '{{ $partner->selectedOption }}' }" class="inline-block w-full overflow-x-auto rounded-lg p-4 shadow-2xl">
              <h2 class="mb-4 text-2xl font-bold">Editando Parceiro #{{ $partner->id }}</h2>
              <form method="POST" action="{{ route('partner.update', ['id' => $partner->id]) }}" class="text-gray-900 dark:text-gray-200">
                @csrf
                @method('PUT')
                <div class="flex flex-wrap">
                  <div class="flex-grow flex-col pr-2">
                    ...
                    <input type="text" id="name" name="name"
                      class="mb-4 w-full rounded-md border p-2 text-gray-900" value="{{ $partner->name }}">
                  </div>

                  <div class="flex-grow flex-col pr-2">
                    <label for="email" class="block w-full">E-mail:</label>
                    <input type="email" id="email" name="email"
                      class="mb-4 w-full rounded-md border p-2 text-gray-900" value="{{ $partner->email }}">
                  </div>
                  ...
                  <div class="flex-grow flex-col pr-2">
                    ...
                    <input type="text" id="telephone" name="telephone"
                      ...
                      placeholder="(99) 9 9999-9999" class="mb-4 w-full rounded-md border p-2 text-gray-900" value="{{ $partner->telephone }}">
                  </div>
                  ...
                  <div class="flex-grow flex-col pr-2" x-show="selectedOption === 'client'">
                    ...
                    <input type="text" id="cgccpf" name="cgccpf" x-mask="999.999.999-99"
                      class="mb-4 w-full rounded-md border p-2 text-gray-900" value="{{ $partner->cgccpf }}">
                  </div>
                  ...
                  <div class="flex-grow flex-col" x-show="selectedOption === 'client'">
                    ...
                    <input type="date" id="birthdate" name="birthdate"
                      class="mb-4 w-full rounded-md border p-2 text-gray-900" value="{{ $partner->birthdate }}">
                  </div>
                  ...
                  <div class="flex-grow flex-col" x-show="selectedOption === 'supplier'">
                    ...
                    <input type="date" id="opening_date" name="opening_date"
                      class="mb-4 w-full rounded-md border p-2 text-gray-900" value="{{ $partner->opening_date }}">
                  </div>

                  <label for="address" class="block w-full">Endereço:</label>
                  <input type="text" id="address" name="address"
                    class="mb-4 w-full rounded-md border p-2 text-gray-900" value="{{ $partner->address }}">
                </div>
